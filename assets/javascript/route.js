    ...
    //Google Maps info window variable declarations:
    // var info = [];
    // var arrayOfInfoWindowElements = [];
    ...
    function ExtractDataFromFirebase() {
      ...
      locations.ref().on("child_added", function(childSnapshot, prevChildKey) {
        ...
        var tempAttraction = (childSnapshot.val().infoWindowContent);
        pointerId.push(temppointerId);
        ptDes.push(tempptDes);
        ptLt.push(tempptLt);
        ptLg.push(tempptLg);
        // if (tempAttraction != undefined) {
        //   info = tempAttraction;
        // }
        //KnjHi_2US2WtDRpHetZ
      },function (error) {
    ...
    //store our only value that contains all the divs in this variable:
    // setTimeout( function () {
    //     info.forEach(function (item) {
    //       arrayOfInfoWindowElements.push(item.toString());
    //     })
    //     console.log("arrayOfInfoWindowElements:   " + arrayOfInfoWindowElements)
    // }, 3000);
